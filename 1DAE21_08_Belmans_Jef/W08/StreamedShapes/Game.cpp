...
Color4f Game::ToColor(const std::string& colorStr) const
{
	...
	while (std::getline(ss, attribute, ','))
	{
		...
		{
		case 0:
			color.r = std::stof(attribute);
			break;
		case 1:
			color.g = std::stof(attribute);
			break;
		case 2:
			color.b = std::stof(attribute);
			break;
		case 3:
			color.a = std::stof(attribute);
			break;
		default:
			std::cerr << "Error getting color attribute from string!\n";
			break;
		}
		index++;
	}
